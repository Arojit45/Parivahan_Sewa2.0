...
const LiveLocationMap = () => {
    ...
    return (
      <div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col">
        ...
        <div className="w-full h-64 rounded-xl border border-slate-200 bg-slate-50 flex flex-col items-center justify-center gap-3 mb-4">
          ...
          <p className="text-sm font-semibold text-slate-400">Location unavailable</p>
          <p className="text-xs text-slate-400">No GPS data found for this vehicle</p>
        </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col">
      ...
      <div>
        <h3 className="font-bold text-slate-900 text-sm">{twin.address ?? "Unknown location"}</h3>
        <div className="flex justify-between items-end mt-1">
          <p className="text-xs text-slate-500 font-medium">
            {twin.speed != null ? `${twin.speed} km/h` : "—"} • {twin.heading ?? "—"}
          </p>
